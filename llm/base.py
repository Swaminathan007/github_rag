...
class BaseModel(ABC):
    ...
    def store_repo_data(self, collection_name: str, repo_data: list[dict]) -> None:
        ...
        self.__logger.info(f"Storing {len(repo_data)} vectors in batches of size {BATCH_SIZE}")
        ...
        for batch_start in range(0, len(repo_data), BATCH_SIZE):
            ...
            try:
                ...
                self.__logger.error(f"Batch {batch_start // BATCH_SIZE + 1} failed: {e}")
                continue
            ...
            for item, vector in zip(batch, vectors):
                ids.append(item["id"])
                payloads.append(
                    {**item.get("metadata", {}), "text": item["text"][:1000]}
                )

            try:
                self.__logger.info(f"Uploading batch starting at {batch_start}")
                ...
                self.__logger.error(
                    f"Upload failed for batch starting at {batch_start}: {e}"
                )
